...
class Repeat(Layer):
    ...
    def call(self, inputs):
        ...
        return outputs
    def get_config(self):
            config = {'repeat_list' : self.repeat_list}
            base_config = super(Repeat, self).get_config()
            return dict(list(base_config.items()) + list(config.items()))

    def compute_output_shape(self, input_shape):
        output_shape = [None]
        for i in range(1,len(input_shape)):#modification
...
def SpatialAttention(inputs,name):
    ...
    attention1 = Conv2D(int(C/2), (1, k), padding='same', name=name+'_1_conv1')(inputs)#modification
    ...
    attention2 = Conv2D(int(C/2), (k, 1), padding='same', name=name+'_2_conv1')(inputs)#modification
...
def ChannelWiseAttention(inputs,name):
    ...
    attention = Dense(int(C / 4), activation='relu')(attention)#modification
